...
新文档位于 [`demo/`](demo/README.md)：九项功能各按“功能介绍 → 机制与本机证据 → SQL 解释 → 场景执行与解释”展开。每项能力都有读取真实数据库结果的数据探查器，正文逐层解释图、查询和使用边界；每个功能页都可从零准备数据、执行独立案例、验收并单独重置，无需先跑其他章节。重点展示百万行大表 Clone 与 CI、向量索引的查询和健康维护、自然语言/布尔/JSON 全文检索；还包括 Stage、外表、ETL、HTAP、快照恢复与 PITR、Publish/Subscribe、Iceberg 和 CDC。13 步业务场景用于串联这些功能。

```bash
./demo/setup.sh   # 首次安装依赖和准备 v4.2.4 源码
./demo/start.sh   # 一键启动数据库与交互文档
```

打开 <http://127.0.0.1:8042/>，从能力目录开始阅读。支持 Debian / Ubuntu 自动安装依赖；`./demo/start.sh --reset` 或网页“从头演示”可清理演示数据后重跑。源码缺失时自动克隆 v4.2.4；路径、依赖和运行范围见 [交互文档说明](demo/README.md)。
