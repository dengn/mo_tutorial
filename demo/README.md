...
每个案例使用固定的 `case_<能力名>_` SQL 命名空间，例如 `case_vector_shop_ci`；数据、账号、任务、快照和 PITR 名称均隔离。状态与实际 SQL 回执放在 `demo/runtime/cases/<能力名>/`。执行时间线按顺序显示 SQL、Shell 和 REST 调用的状态、响应与耗时，长调用的计时会持续更新；验收区单独对照预期与实际结果，失败时保留已取得的检查数据。共享数据库服务、MinIO/Nessie 与 MySQL 容器，但 CDC 目标库和湖表命名空间独立。
